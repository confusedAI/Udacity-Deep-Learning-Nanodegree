...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        hidden_inputs =  np.dot(np.reshape(X,(1,X.shape[0])),self.weights_input_to_hidden) # signals into hidden layer
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        delta_weights_i_h += np.dot(np.reshape(X,(X.shape[0],1)),hidden_error_term)
        # Weight step (hidden to output)
        delta_weights_h_o += np.dot(hidden_outputs.T,error)
